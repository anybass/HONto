...
from concept_formation.owncobweb import CobwebNode
...
class Cobweb3Node(CobwebNode):
    ...
    def probability(self, attr, val, c, fpdict, tpdict, fndict):
        """
        ...
        if val is None:
            ...
            if attr in self.av_counts:
                ct = sum([self.av_counts[attr][v].num if v == cv_key
                         else self.av_counts[attr][v] for v in
                         self.av_counts[attr]])
            return (self.count - ct) / self.count
        ...
        if attr in self.av_counts and val == c:

            #tp counter + 1 für val (und tn + 1 für alle anderen klassen, ist nicht wichtig hier)
            if val in self.av_counts[attr]:
              #  print("here tp: " + val + " " + str(self.av_counts[attr]) + " " + c)
                #print(str(self.av_counts[attr][val] / self.count) + " " + str(self.av_counts[attr][val]) + " " +  str(self.count) + " " +  attr + " " +  val + " " +  c)
          #new: returning different types by using a list, still has to be adapted in calling modules
                tpdict[c] += 1.0
              #  print(tpdict)
                return [self.av_counts[attr][val] / self.count, fpdict, tpdict, fndict]
            else:
              #  print("here fn: " + val + " " + str(self.av_counts[attr]) + " " + c)
                # print(str(self.av_counts[attr]) + str(self.count) + " " + attr + " " + val + " " + c)
                fndict[c] += 1.0
              #  print(fndict)
                return [0.0, fpdict, tpdict, fndict]
        elif attr in self.av_counts and  c in self.av_counts[attr]:
           # print("here fp: " + val + " " + str(self.av_counts[attr]) + " " + c )
            fpdict[c]+=1.0
          #  print(fpdict)
            ...
            return 0.0
        #fp + 1 für val und fn +1 av_counts classification key
