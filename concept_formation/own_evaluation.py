...
from concept_formation.own_structure_mapper import StructureMapper
from concept_formation.owncobweb3 import Cobweb3Tree
from concept_formation.owndummy import DummyTree

def probability(tree, instance, attr, val, c, fpdict, tpdict, fndict):
    """
    ...
    if isinstance(val, dict):
        ...
        print("here if")
        probs = [concept.probability(sub_attr, temp_instance[sub_attr], c)
                 for sub_attr in temp_instance
                 if search(r'^' + mapping[attr], sub_attr)]
        return mean(probs)
    else:
        return concept.probability(attr, val, c, fpdict, tpdict, fndict)
...
def incremental_evaluation(tree, instances, classlist, attr, run_length, runs=1,
    ...
    * :func:`squared_error` - Returns the error squared.
    ...
    for c in classlist:
        ...
        for r in range(runs):
            ...
            for i, instance in enumerate(instances[:run_length + 2]):
               ...
               # print(i)
                ...
                if attr in instance:
                    val = instance[attr]
                resultlist.append(score(tree, instance, attr, val, c, fpdict, tpdict, fndict))
            #    print(attr, val, c, str(fpdict), str(tpdict), str(fndict) )
                #print(i)
                #print("resultlist")
                #print(resultlist)

                if resultlist[i] == 0.0:
                    row.append(resultlist[i])
                elif len(resultlist)>1:

                    if resultlist[i][0] !=0.0:

                        row.append(resultlist[i][0])
                        tpdict=resultlist[i][2]
                    elif resultlist[i][0] ==0.0:
                        row.append(resultlist[i][0])
                        fpdict=resultlist[i][1]
                        fndict=resultlist[i][3]
                if tpdict[c]+fpdict[c] != 0:
                    precision = tpdict[c]/(tpdict[c]+fpdict[c])
               ...
               # print("precision" + str(precision))
                if tpdict[c] + fndict[c] != 0:
                    recall = tpdict[c] / (tpdict[c] + fndict[c])
                else:
                    recall = 0.0
               # print("recall" + str(recall))
                ...
                rlist.append(recall)
            precisionlist.append(plist)
            recalllist.append(rlist)
            scores.append(row)
        print("current class evaluation: " + c)
        print(precisionlist)
       # input()
        print(recalllist)
          #  print("scores")
       ...
       # scorelist.append(scores)
        scorelist.append(precisionlist)
        scorelist.append(recalllist)
